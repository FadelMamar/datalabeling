...
def get_preds_targets(
    ...
    if images_paths is not None:
        ...
        df_labels, col_names = load_groundtruth(
    ...
    for image_dir in images_dirs:
        ...
        if features_names is None:
            ...
            check_changes = len(
                set(features_names).intersection(set(col_names))
            ) == len(features_names)
...
def compute_detector_performance(
    df_results: pd.DataFrame, df_labels: pd.DataFrame, col_names: list[str]
):
    ...
    for image_path in tqdm(image_paths, desc="Computing metrics"):
        ...
        target_list = [
            {
                "boxes": torch.from_numpy(get_bbox(gt=gt)),
...
def select_hard_samples(
    ...
    df_results_per_img = get_uncertainty(
        df_results_per_img=df_results_per_img, mode=uncertainty_method
    )

    mask_low_map = (df_results_per_img["map50"] < map_thrs) * (
        df_results_per_img["map75"] < map_thrs
    )
    ...
    mask_selected = (
        ...
        + (df_results_per_img["uncertainty"] > uncertainty_thrs)
    )
