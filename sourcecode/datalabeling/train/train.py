...
def sample_pos_neg(images_paths: list, ratio: float, seed: int = 41):
    ...
    # build dataframe
    is_empty = [1 - Path(str(p).replace('images', 'labels')
                         ).with_suffix('.txt').exists() for p in images_paths]
...
def get_data_cfg_paths_for_HN(args: Arguments, data_config_yaml: str):
    ...
    from ..dataset.sampling import (
        get_preds_targets, compute_detector_performance, get_uncertainty)
    ...
    images_paths = pd.read_csv(images_paths, header=None, names=[
                               'paths'])['paths'].to_list()
    ...
    df_results_per_img = compute_detector_performance(
        df_results, df_labels, col_names)
    ...
    mask_selected = mask_low_map * mask_high_scores + mask_low_map * \
        mask_low_scores + \
...
def continual_learning_run(model: YOLO, args: Arguments, img_glob_pattern: str = "*"):
    ...
    for flag in (args.cl_ratios, args.cl_epochs, args.cl_freeze):
        assert len(flag) == len(
            args.cl_lr0s), "all args.cl_* flags should have the same length."
